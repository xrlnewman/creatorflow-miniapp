...
export function createApiClient({ baseUrl, fetchImpl = globalThis.fetch } = {}) {
  ...
  const configuredBase = baseUrl ?? globalThis.__CREATORFLOW_API_BASE__ ?? import.meta.env?.VITE_API_BASE_URL
  ...
  async function request(path, { method = 'GET', body, idempotencyKey, headers = {} } = {}) {
    ...
    try {
      ...
      throw new Error(`无法连接 CreatorFlow 服务：${error instanceof Error ? error.message : '网络异常'}`)
    }
